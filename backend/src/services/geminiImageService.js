...
const { logApiCall, logApiResponse, logError, logRetry } = require('../utils/workflowLogger');
...
class GeminiImageService {
  ...
  async generateImage(prompt, options = {}) {
    ...
    try {
      ...
      if (error.response) {
        ...
        logger.debug(`   Status Text: ${error.response.statusText}`);
   ...
   */
  async _makeRequestWithRetry(payload, timeout) {
    let lastError;
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        if (error.response) {
          logger.debug(`   Status: ${error.response.status}`);
          logger.debug(`   Status Text: ${error.response.statusText}`);
          logger.debug(`   Response Headers:`, JSON.stringify(error.response.headers, null, 2));
          logger.debug(`   Response Data:`, JSON.stringify(error.response.data, null, 2));
   ...
   */
  _extractImageFromResponse(responseData) {
 ...
 *
 * @param {string} apiKey - Google Gemini API key
 * @returns {GeminiImageService} Service instance
 */
function createGeminiImageService(apiKey) {
