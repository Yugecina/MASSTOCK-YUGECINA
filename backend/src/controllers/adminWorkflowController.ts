...
import { updateWorkflowStageSchema } from '../validation/schemas';
...
export async function getWorkflows(req: Request, res: Response): Promise<void> {
  ...
  const workflowsWithStats = workflows.map((workflow: any) => {
    const workflowExecutions = executionsData.filter((e: any) => e.workflow_id === workflow.id);
    const totalExecutions = workflowExecutions.length;
    const successCount = workflowExecutions.filter((e: any) => e.status === 'completed').length;
    const errorCount = workflowExecutions.filter((e: any) => e.status === 'failed').length;
...
export async function getWorkflow(req: Request, res: Response): Promise<void> {
  ...
  const failureCount = executions?.filter((e: any) => e.status === 'failed').length || 0;

  // Calculate average duration
  const completedExecutions = executions?.filter((e: any) => e.status === 'completed' && e.duration_seconds) || [];
  const avgDuration = completedExecutions.length > 0
    ? completedExecutions.reduce((sum: number, e: any) => sum + (e.duration_seconds || 0), 0) / completedExecutions.length
    : 0;
 ...
 */
export async function updateWorkflowRequestStage(req: Request, res: Response): Promise<void> {
  try {
    const { id } = req.params;
...
export async function getWorkflowStats(req: Request, res: Response): Promise<void> {
  ...
  const successCount = allExecutions.filter((e: any) => e.status === 'completed').length;
  const failureCount = allExecutions.filter((e: any) => e.status === 'failed').length;
  const pendingCount = allExecutions.filter((e: any) => e.status === 'pending').length;
  const processingCount = allExecutions.filter((e: any) => e.status === 'processing').length;
  ...
  const completedExecutions = allExecutions.filter((e: any) => e.status === 'completed' && e.duration_seconds);
  const avgDuration = completedExecutions.length > 0
    ? completedExecutions.reduce((sum: number, e: any) => sum + (e.duration_seconds || 0), 0) / completedExecutions.length
    : 0;
  ...
  const monthlyExecutions = allExecutions.filter((e: any) =>
    new Date(e.created_at) >= startOfMonth
  );

  const monthlySuccess = monthlyExecutions.filter((e: any) => e.status === 'completed').length;
