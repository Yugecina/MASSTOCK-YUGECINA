...
import { z } from 'zod';
import { supabaseAdmin, getCleanAdmin } from '../config/database';
...
export async function getAssets(req: Request, res: Response): Promise<void> {
  try {
    ...
    const executionIds = [...new Set(items.map((a: any) => a.workflow_executions.id))];
    const workflowIds = [...new Set(items.map((a: any) => a.workflow_executions.workflow_id))];
    ...
    const formattedAssets = items.map((asset: any) => {
    ...
    if (typeCountsResult.data) {
      ...
      typeCountsResult.data.forEach((item: any) => {
    ...
    if (error instanceof z.ZodError) {
      logger.warn('⚠️ getAssets: Validation error:', { errors: error.issues });
      res.status(400).json({
        success: false,
