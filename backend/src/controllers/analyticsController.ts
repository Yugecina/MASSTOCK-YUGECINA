 ...
 */
function parsePeriod(period: string = '7d'): number {
...
export async function getOverview(req: Request, res: Response): Promise<void> {
  try {
    ...
    const revenueThisMonth = parseFloat(String(revenueThisMonthData?.total_revenue || 0));
    ...
    const revenueLastMonth = parseFloat(String(revenueLastMonthData?.total_revenue || 0));
 ...
 */
export async function getExecutionsTrend(req: Request, res: Response): Promise<void> {
  const { period = '7d' } = req.query;
  const days = parsePeriod(period as string);
 ...
 */
export async function getWorkflowPerformance(req: Request, res: Response): Promise<void> {
  ...
  const transformedData = (data || []).map((workflow: any) => ({
    id: workflow.id,
    name: workflow.name,
...
export async function getRevenueBreakdown(req: Request, res: Response): Promise<void> {
  ...
  // Validate type
  if (!['client', 'workflow'].includes(type as string)) {
...
export async function getFailures(req: Request, res: Response): Promise<void> {
  ...
  const transformedData = (data || []).map((failure: any) => ({
    id: failure.id,
