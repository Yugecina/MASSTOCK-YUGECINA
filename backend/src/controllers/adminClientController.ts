 ...
 */
export async function addClientMember(req: Request, res: Response): Promise<void> {
  try {
    const { client_id } = req.params;
    ...
    if (error instanceof z.ZodError) {
      logger.error('AdminClientController.addClientMember validation error:', { errors: error.issues });
      res.status(400).json({
        success: false,
 ...
 * Update member role
 */
export async function updateClientMemberRole(req: Request, res: Response): Promise<void> {
  try {
    ...
    // Handle Zod validation errors
    if (error instanceof z.ZodError) {
      logger.error('AdminClientController.updateClientMemberRole validation error:', { errors: error.issues });
      res.status(400).json({
        success: false,
        message: 'Validation error',
        errors: error.issues
 ...
 */
export async function removeClientMember(req: Request, res: Response): Promise<void> {
  const { client_id, member_id } = req.params;
 ...
 */
export async function getClientWorkflows(req: Request, res: Response): Promise<void> {
  const { client_id } = req.params;
  ...
  const workflows = (accessList
    ?.map((a: any) => a.workflow)
    .filter((w: any) => w && w.status !== 'archived')
    .sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()) || []) as any[];
 ...
 */
export async function assignWorkflowToClient(req: Request, res: Response): Promise<void> {
  try {
    const { client_id } = req.params;
    ...
    if (!sharedWorkflow) {
      ...
      const { data: newWorkflow, error: insertError } = await supabaseAdmin
        ...
        .insert([{
          ...
          is_shared: true,
          created_by_user_id: (req as any).user.id,
    ...
    // Handle Zod validation errors
    if (error instanceof z.ZodError) {
      logger.error('AdminClientController.assignWorkflowToClient validation error:', { errors: error.issues });
      res.status(400).json({
        success: false,
        message: 'Validation error',
        errors: error.issues
 ...
 */
export async function removeClientWorkflow(req: Request, res: Response): Promise<void> {
 ...
 */
export async function getClientExecutions(req: Request, res: Response): Promise<void> {
  const { client_id } = req.params;
  const {
    workflow_id,
  ...
  // Pagination
  const offset = (parseInt(String(page || '1')) - 1) * parseInt(String(limit || '20'));
  query = query.range(offset, offset + parseInt(String(limit || '20')) - 1);
  ...
  res.json({
    ...
    data: {
      ...
      pagination: {
        ...
        total_pages: Math.ceil((count || 0) / parseInt(String(limit || '20')))
      }
    }
  });
}
 ...
 */
export async function getClientActivity(req: Request, res: Response): Promise<void> {
  const { client_id } = req.params;
  ...
  const { data: logs, error, count } = await supabaseAdmin
    ...
    .eq('client_id', client_id)
    .order('created_at', { ascending: false })
    .range(parseInt(String(offset || '0')), parseInt(String(offset || '0')) + parseInt(String(limit || '50')) - 1);
