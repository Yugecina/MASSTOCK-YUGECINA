...
export function AdminDashboard() {
  ...
  const getActivityIcon = (action: string): ReactElement => {
  ...
  const formatActivity = (activity: Activity): FormattedActivity => {
    const actionMap: Record<string, { label: string; color: 'success' | 'neutral' | 'error' | 'warning' }> = {
  ...
  const navigateToResource = (activity: Activity): void => {
    ...
    const route = activity.resource_type ? routes[activity.resource_type] : null
    if (route) {
      navigate(route)
    }
  }
