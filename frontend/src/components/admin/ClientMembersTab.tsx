...
import type { ClientTabProps, ClientMember } from '../../types/admin';

/**
 * ClientMembersTab - List and manage client members
 */
export function ClientMembersTab({ clientId }: ClientTabProps) {
  ...
  const handleRoleChange = async (memberId: string, newRole: 'owner' | 'collaborator') => {
  ...
  const handleRemove = async (memberId: string, email: string | undefined) => {
  ...
  // Type guard to ensure members are ClientMember[]
  const clientMembers = (members || []) as ClientMember[];

  const owners = clientMembers.filter((m) => m.role === 'owner');
  const collaborators = clientMembers.filter((m) => m.role === 'collaborator');
      ...
      <div className="admin-section-header">
        <div>
          ...
          <p className="admin-section-subtitle">
            {clientMembers.length} member{clientMembers.length !== 1 ? 's' : ''} ({owners.length} owner{owners.length !== 1 ? 's' : ''}, {collaborators.length} collaborator{collaborators.length !== 1 ? 's' : ''})
          </p>
        </div>
            ...
            <div key={member.id} className="admin-member-row">
              <div className="admin-member-info">
                ...
                <div className="admin-member-details">
                  ...
                  <span className="admin-member-meta">
                    ...
                    {member.user && 'last_login' in member.user && member.user.last_login && (
              ...
              <div className="admin-member-actions">
                <select
                  ...
                  value={member.role}
                  onChange={(e: ChangeEvent<HTMLSelectElement>) => handleRoleChange(member.id, e.target.value as 'owner' | 'collaborator')}
