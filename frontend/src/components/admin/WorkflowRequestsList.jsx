...
const TERMINAL_STAGES = ['deployed', 'rejected'];

export function WorkflowRequestsList({ requests = [], onUpdateStage, loading = false }) {
  ...
  const handleStageChange = (requestId, newStage) => {
    try {
                ...
                <div className="border-t" style={{ paddingTop: 'var(--spacing-md)', marginTop: 'var(--spacing-sm)' }}>
                  <div className="flex items-center gap-md">
                    ...
                    <select
                      ...
                      onChange={(e) => handleStageChange(request.id, e.target.value)}
                      disabled={loading}
                      style={{
                        padding: '8px 12px',
