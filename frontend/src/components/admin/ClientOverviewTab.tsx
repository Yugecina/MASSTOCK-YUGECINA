...
import type { ClientTabProps, ClientStats, ClientDetailResponse } from '../../types/admin';
...
/**
 * Type guard to check if client has ClientDetailResponse shape
 */
function isClientDetailResponse(client: any): client is ClientDetailResponse {
  return client && typeof client === 'object' && 'client' in client && 'stats' in client;
}

/**
 * Extract client data from either Client or ClientDetailResponse
 */
function extractClientData(client: Client | ClientDetailResponse | null): ClientDetailResponse['client'] | null {
  if (!client) return null;
  if (isClientDetailResponse(client)) {
    return client.client;
  }
  return client as ClientDetailResponse['client'];
}

/**
 * Extract stats from ClientDetailResponse or return empty stats
 */
function extractStats(client: Client | ClientDetailResponse | null): ClientStats {
  if (!client) {
    return {
      total_workflows: 0,
      total_executions: 0,
 ...
 */
export function ClientOverviewTab({ clientId }: ClientTabProps) {
  ...
  const stats = extractStats(client);
